...
import { BuildScreen, ConnectRecordScreen, ProcessingScreen, PreviewScreen, ReviewScreen, TalkThroughScreen, UploadScreen } from "@/components/OnboardingScreens";
...
export default async function OnboardingStepPage({ params }: { params: Promise<{ step: string }> }) {
  ...
  if (step === "life-map") {
    const [anchors, frictions] = await Promise.all([
