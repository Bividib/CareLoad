...
export async function POST(request: Request) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    const message = await tx.message.create({ data: { id: `message-clarification-${suffix}`, threadId: thread.id, patientId: "eleanor-reed", author: "PATIENT", body: parsed.data.body } });
